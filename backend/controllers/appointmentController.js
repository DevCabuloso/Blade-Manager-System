...
const sanitizeCustomerName = (value) => {
  const validation = validateDisplayName(value, { label: 'Nome do cliente', maxLength: 80 });
...
const createGuestCustomer = async (guestName) => {
  ...
  const { data, error } = await supabase
    ...
    .insert([
      {
        nome_usuario: guestName || 'Cliente visitante',
...
export const getAppointments = async (req, res) => {
  ...
  try {
    ...
    let customerNamesById = new Map();

    if (userType === 'barbeiro') {
      const customerIds = [...new Set((data || []).map((appointment) => appointment?.cliente_id).filter(Boolean))];
      ...
      if (customerIds.length > 0) {
        ...
        if (customerError) {
          console.error('Supabase error fetching customer names for appointments:', customerError);
        } else {
          customerNamesById = new Map((customers || []).map((customer) => [customer.id, customer.nome_usuario]));
        }
      }
    }
    ...
    const rows = (data || []).map((a) => ({
      ...
      time: a.data_hora,
      status: a.status,
      customerName: userType === 'barbeiro' ? customerNamesById.get(a.cliente_id) || null : null,
    }));
...
export const createAppointment = async (req, res) => {
  const { servico_id, data_hora, barbeiro_id, nome_cliente } = req.body;
  let cliente_id = req.user?.id || null;
  let createdGuestUserId = null;
  const sanitizedCustomerName = sanitizeCustomerName(nome_cliente);
  ...
  try {
    ...
    if (!cliente_id) {
      const { guestUserId, error: guestError } = await createGuestCustomer(sanitizedCustomerName);
...
export const deleteAppointment = async (req, res) => {
  ...
  const appointmentId = parseStrictInteger(req.params?.id);
  if (!appointmentId) {
    return res.status(400).json({ message: 'Identificador de agendamento invalido.' });
  }
  ...
  try {
    ...
    if (fetchError) {
      console.error('Supabase error fetching appointment for delete:', fetchError);
      return res.status(500).json({ message: 'Erro interno.' });
    }
    ...
    const requesterId = parseStrictInteger(req.user.id);
    const requesterType = String(req.user.tipo_usuario || '').trim().toLowerCase();

    if (requesterType !== 'admin' && appointment.barbeiro_id !== requesterId) {
    ...
    if (deleteError) {
      console.error('Supabase error deleting appointment:', deleteError);
      return res.status(500).json({ message: 'Erro interno.' });
    }
