    ...
    >
      ...
      <v-list nav density="comfortable" class="layout-nav">
        <v-list-item
          ...
          :class="{ 'layout-nav-item--collapsed': !isMobile && isCollapsed }"
    ...
    <div class="layout-main">
      <v-app-bar flat color="rgba(2, 6, 23, 0.82)" class="layout-header">
        <AppButton variant="secondary" rounded="xl" class="layout-menu-action" @click="toggleSidebar">
          {{ isMobile ? 'Menu' : isCollapsed ? 'Expandir' : 'Recolher' }}
        </AppButton>
...
import { clearAuthSession, getSidebarCollapsed, getStoredUser, setSidebarCollapsed } from '@/utils/authStorage.js';
...
.layout-nav-item--collapsed :deep(.v-list-item__prepend) {
  margin-inline-end: 0;
}
...
.layout-nav-item--collapsed :deep(.v-list-item__prepend > .v-icon) {
  width: 2.15rem;
  height: 2.15rem;
}
